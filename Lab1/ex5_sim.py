...
if __name__ == '__main__':
    ...
    DISTRIBUTIONS = ['Exponential','Uniform','Constant','Gaussian']
    ...
    for i in range(len(tot_loss_pr)):
        plt.plot(load_list, tot_loss_pr[i], '-', linewidth=0.7, c=colors[1][i], label=f'Simluated B={DISTRIBUTIONS[i]}')
    ...
    for i in range(len(tot_loss_pr)):
        plt.plot(load_list, tot_loss_pr[i], '.-', linewidth=0.5, c=colors[1][i], label=f'Simluated B={DISTRIBUTIONS[i]}')
        #plt.plot(load_list, tot_pB_list[i], linewidth=0.5, c=colors[0][i], label=f'Theoretical B={DISTRIBUTIONS[i]}')
    plt.grid()
    plt.legend()
    plt.xlabel("Load")
    plt.ylabel("Loss probability")
    ...
    for i in range(len(tot_time_sys)):
        plt.plot(load_list, tot_time_sys[i], '.-', linewidth=0.5, c=colors[1][i], label=f'Simulated B={DISTRIBUTIONS[i]}')
        #plt.plot(load_list, tot_th_av_time_sys_list[i], linewidth=1, c=colors[0][i], label=f'Theoretical B={DISTRIBUTIONS[i]}')
    plt.grid()
    plt.legend()
    plt.xlabel("Load")
